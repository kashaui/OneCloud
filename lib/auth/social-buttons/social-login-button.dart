...
class LoginWithGoogleButton extends StatelessWidget {
  const LoginWithGoogleButton(this.provider, {
    Key key,
  }) : super(key: key);
...
_loginWithGoogle(BuildContext context) async {
  ...
  try {
    ...
    String msg = e is BackendError ? e.message : GENERIC_ERROR_MESSAGE;
...
class _Icon extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    String logo = provider == SocialLoginProvider.apple ? 'apple-white' : provider.value;
    return SvgPicture.asset('assets/social-logos/$logo-logo.svg',
      height: 26,
      width: 26,
    );
  }
