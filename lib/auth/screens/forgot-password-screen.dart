...
class ForgotPasswordScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return AuthScreenWrapper(
      title: text('Enter your email address below and we will send you a link to reset or create your password.', singleLine: false, height: 1.2),
      screen: Form(
        key: _formKey,
        child: Column(
          ...
          children: [
            TextFormField(
              ...
              onFieldSubmitted: loading ? null : (_) => _sendResetLink(context),
              validator: (value) {
  ...
  _sendResetLink(BuildContext context) async {
    ...
    if (_formKey.currentState.validate()) {
      ...
      try {
        final message = await context.read<AuthState>().sendPasswordResetLink(formPayload['email']);
        Navigator.of(context).pushReplacementNamed(LoginScreen.ROUTE, arguments: AuthScreenArgs(message));
