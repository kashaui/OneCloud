...
class AuthState with ChangeNotifier {
  ...
  Future<AuthState> init() async {
    _setUser(await localStorage.permanent.get(CURRENT_USER_FILE_NAME), storeLocally: false);
    httpClient.setErrorHandler((BackendError e) {
      if (e.status == 401 && rootNavigatorKey.currentState != null) {
        logout(rootNavigatorKey.currentContext, message: 'Your session has expired. Please sign in again.');
      }
    });
    return this;
  }
  ...
  Future<String> sendPasswordResetLink(String email) async {
    toggleLoading(true);
    try {
      return (await this.httpClient.post('/auth/password/email', payload: {'email': email}))['data'];
    } finally {
      toggleLoading(false);
    }
  }

  loginWithSocialService(SocialLoginProvider provider, String accessToken, BuildContext context) async {
    toggleLoading(true);
    try {
      final response = BootstrapDataResponse.fromJson(
        await httpClient.get(
            '/auth/social/${provider.value}/callback',
            params: {'tokenForDevice': await deviceIdentifier(), 'tokenFromApi': accessToken}
        )
      );
      _setUser(response.data.user);
    } finally {
  ...
  _setUser(dynamic userData, {bool storeLocally = true}) async {
    if (userData != null) {
      ...
      if (storeLocally) {
        await localStorage.permanent.put(CURRENT_USER_FILE_NAME, currentUser.toRawJson());
      }
  ...
  _maybeUpdateFirebaseToken() async {
    ...
    if (fcmToken != currentUser.fcmToken) {
      httpClient.post('/fcm-token', payload: {'token': fcmToken, 'deviceId': await deviceIdentifier()}).catchError((_) {});
      currentUser.fcmToken = fcmToken;
      localStorage.permanent.put(CURRENT_USER_FILE_NAME, currentUser.toRawJson());
    }
  }
}