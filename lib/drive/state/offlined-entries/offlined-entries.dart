...
class OfflinedEntries with ChangeNotifier {
  ...
  OfflinedEntries(this.entriesApi, this.transferQueue) {
    ...
    localStorage = entriesApi.localStorage.permanent.scopedToSubDir(OfflinedEntries.filesPath);
  ...
  offline(List<FileEntry> entries, {BuildContext context}) async {
    entries = (await _fetchChildEntries(entries)).where((e) => !offlinedEntryIds.contains(e)).toList();
  ...
  sync() async {
    ...
    localStorage.rootDir.listSync(recursive: true, followLinks: false).forEach((e) {
      if (e is File) fileNames.add(basename(e.path));
    });
    if (fileNames.isNotEmpty) {
      List<String> syncInfo = (await entriesApi.getSyncInfo(fileNames)).map((e) => e.fileName).toList();
      List<String> filesToDelete = fileNames..removeWhere((e) => syncInfo.contains(e));
      filesToDelete.forEach((e) => localStorage.delete(e));
      List<Map<String, dynamic>> result = await (await db).query('entries', where: 'file_name in (?)', whereArgs: [filesToDelete.join(',')]);
      List<int> deletedEntryIds = List.from(result.map((e) => e['id']));
      (await db).delete('entries', where: 'file_name in (?)', whereArgs: [filesToDelete.join(',')]);
      offlinedEntryIds.removeWhere((e) => deletedEntryIds.contains(e));
    }
  }

  Future<Map<String, dynamic>> loadEntries(int page, {Map<String, String> params}) async {
    ...
    final result = await (await db).query(
      ...
      offset: offset,
      limit: limit,
      orderBy: '${ReCase(params['orderBy']).snakeCase} ${ReCase(params['orderDir']).snakeCase}',
      where: 'parent_id $parentIdOperator $parentId',

    );
    final total = Sqflite.firstIntValue(await (await db).rawQuery('SELECT COUNT(*) FROM entries where parent_id $parentIdOperator $parentId'));
    return {
      'current_page': page,
      'from': offset,
  ...
  Future<List<dynamic>> _query({List<int> parentIds, String column}) async {
    ...
    if (parentIds == null) {
      ...
      result = await (await db).query('entries', where: 'parent_id in (?)', whereArgs: [parentIds.join(',')], columns: columns);
    }
    return List.from(result.map((e) => column == null ? FileEntry.fromJson(e) : e[column]));
  }
  ...
  Future<List<FileEntry>> _delete(List<FileEntry> entries) async {
    List<int> parentIds = entries.where((e) => e.type == 'folder').map((e) => e.id).toList();
    List<int> childIds = parentIds.isNotEmpty ? List<int>.from((await _query(parentIds: parentIds))) : [];
    List<int> allIds = entries.map((e) => e.id).toList()..addAll(childIds);
    List<FileEntry> allEntries = List<FileEntry>.from((await _query()));
    (await db).delete('entries', where: 'id in (?)', whereArgs: [allIds.join(',')]);
    return allEntries;
  }
  ...
  Future<List<FileEntry>> _fetchChildEntries(List<FileEntry> entries) async {
    ...
    if (folders.isNotEmpty) {
      try {
        ...
        final childEntries = List.from(json.decode(response)['data']).map((e) => FileEntry.fromJson(e));
  ...
  void _bindBackgroundIsolate() {
    bool isSuccess = IsolateNameServer.registerPortWithName(_syncPort.sendPort, OfflinedEntries.syncTaskName);
    if (!isSuccess) {
