...
class DriveState with ChangeNotifier {
  ...
  StreamSubscription<FileEntry> transferQueueSub;
  ...
  Future<FileEntry> renameFile(FileEntry fileEntry, {String name, String description}) async {
    final entry = await api.renameFile(fileEntry.id, name: name, description: description);
    entryCache.set(entry.id, entry);
    notifyListeners();
    return entry;
  }

  unshare(List<int> entryIds, int userId) async {
    final users = await api.removeUser(entryIds, userId);
    if (activePage is SharesPage) {
      removeEntries(entryIds);
    }
    return updateEntryUsers(entryIds, users);
  }

  List<FileEntryUser> updateEntryUsers(List<int> entryIds, List<FileEntryUser> users) {
    entryIds.forEach((id) => entryCache.get(id).users = users);
    notifyListeners();
    return users;
  }

  Future<void> deleteEntries(List<int> entryIds, {bool deleteForever = false}) async {
  ...
  Future<void> restoreEntries(List<int> entryIds, {bool deleteForever = false}) async {
  ...
  Future<List<FileEntry>> moveOrCopyEntries(List<FileEntry> targetEntries, int destinationId, EntryMoveType moveType) async {
    final targetIds = targetEntries.map((f) => f.id).toList();
    final response = await api.moveOrCopyEntries(targetIds, destinationId, moveType);
  ...
  addEntries(List<FileEntry> newEntries) {
    if (newEntries.isNotEmpty) {
      ...
      int folderCount = newEntriesList.where((id) => entryCache.get(id).type == 'folder').length;
  ...
  Future<void> loadEntries(int page) async {
    ...
    try {
      ...
      final liveResponse = skipLoad ? await Future.value(null) : await api.loadEntries(params, cancelToken: cancelToken);
      cancelToken = null;
 ...
 void  _handleLoadEntriesResponse(int page, dynamic response) {
    pagination = response == null ? Pagination.empty() : Pagination.fromJson(response);
