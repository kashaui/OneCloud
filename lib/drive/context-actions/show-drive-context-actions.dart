...
Future<DriveContextAction> showDriveContextActions(List<FileEntry> entries, {@required BuildContext context, bool hidePreview = false}) {
  final state = context.read<DriveState>();
  final onlyIncludesFolders = entries.every((e) => e.type == 'folder');
  List<DriveContextActionConfig> actions = state.activePage.contextActions.map((a) => a.getConfig(context, entries)).toList();

  actions = actions..retainWhere((a) {
    final showPreview = (a.name != DriveContextAction.preview) || (!hidePreview && !onlyIncludesFolders);
    final hideOnIos = a.name == DriveContextAction.download && Platform.isIOS;
    final hasPermissions = a.permission == null || entries.every((e) => e.permissions.contains(a.permission));
