...
class SelectionModeAppBar extends StatelessWidget{
  ...
  Widget build(BuildContext context) {
    ...
    final selectedEntries = selectedIds.map((id) => cache.get(id)).toList();
    final moveAction = DriveContextAction.move.getConfig(context, selectedEntries);
    ...
    return AppBar(
      ...
      title: text(':count Selected', replacements: {'count': selectedEntries.length.toString()}, weight: FontWeight.normal, size: 18),
      actions: [
        IconButton(
        ...
        IconButton(icon: const Icon(Icons.more_vert_outlined), onPressed: () async {
