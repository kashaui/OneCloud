...
class _CurrentUser extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return UserAccountsDrawerHeader(
      ...
      accountName: text(currentUser.displayName, translate: false),
      accountEmail: text(currentUser.email, translate: false),
      currentAccountPicture: CircleAvatar(
        radius: 15,
        backgroundImage: currentUser.avatar != null ? CachedNetworkImageProvider(http.prefixUrl(currentUser.avatar)) : null,
        child: currentUser.avatar == null ? Text(currentUser.displayName.substring(1, 3)) : null,
      ),
    );
  }
}
