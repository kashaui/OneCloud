...
class FileGridItem extends StatelessWidget with FileViewTileActions {
  ...
  @override
  Widget build(BuildContext context) {
    final isDestinationPicker = context.select((DestinationPickerState s) => s.active);
    final isSelected = !isDestinationPicker && context.select((DriveState s) => s.selectedEntries.contains(fileEntry.id));
    final isDisabled = isDestinationPicker && (fileEntry.type != 'folder' || !fileEntry.permissions.contains(EntryPermission.update));
                ...
                children: [
                  Expanded(
                    child: ClipRRect(
                      ...
                      borderRadius: BorderRadius.circular(4),
                      child: FileThumbnail(fileEntry, size: FileThumbnailSize.big),
                    ),
                  ),
...
class FileGridItemFloatingInfo extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    bool isStarred = context.select((DriveState s) => s.entryCache.get(fileEntry.id).isStarred());
    bool isShared = context.select((DriveState s) => s.entryCache.get(fileEntry.id).users.length > 1);
    bool isOfflined = context.select((OfflinedEntries s) => s.offlinedEntryIds.contains(fileEntry.id));
