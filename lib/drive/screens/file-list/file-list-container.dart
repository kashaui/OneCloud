...
class _Body extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final loadingFromBackend = context.select((DriveState s) => s.isLoadingFromBackend);
    ...
    if (isInitialLoad && loadingFromBackend) {
      ...
      return SliverPadding(
        ...
        sliver: mode == FileListMode.grid ? FileGrid(entries) : FileList(entries)
      );
    }
  }
}
...
class _Footer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final lastBackendError = context.select((DriveState s) => s.lastBackendError);
    if (isLoadingNewPage) {
      return Container(
