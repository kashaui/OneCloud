...
class Body extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    final initialLoadCompleted = context.select((ShareableLinkState s) => s.initialLoadCompleted);
    return initialLoadCompleted == false ? Container() : Container(
...
class ExpirationFormField extends StatelessWidget {
    ...
    return Column(
      ...
      children: [
        CheckboxListTile(
          ...
          title: text('Link expiration', style: Theme.of(context).textTheme.subtitle1),
  ...
  _formField(ShareableLink link, BuildContext context) {
    ...
    return DateTimePicker(
      onSaved: (v) => context.read<ShareableLinkState>().formPayload['expiresAt'] = v,
      type: DateTimePickerType.dateTime,
      initialValue: form['expiresAt'] != null ? form['expiresAt'].toString() : null,
...
class PasswordFormField extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    final passwordEnabled = context.select((ShareableLinkState s) => s.passwordEnabled);
    final link = context.select((ShareableLinkState s) => s.link);
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        CheckboxListTile(
          contentPadding: EdgeInsets.all(0),
          title: text('Require password', style: Theme.of(context).textTheme.subtitle1),
  ...
  TextFormField _formField(ShareableLink link, BuildContext context) {
    ...
    return TextFormField(
        onChanged: (v) => context.read<ShareableLinkState>().formPayload['password'] = v,
        obscureText: true,
...
class AllowEditingField extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    bool allowEdit = context.select((ShareableLinkState s) => s.formPayload['allowEdit'] ?? false);
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        CheckboxListTile(
          contentPadding: EdgeInsets.all(0),
          title: text('Allow editing', style: Theme.of(context).textTheme.subtitle1),
          value: allowEdit,
          onChanged: (value) {
...
class AllowDownloadFormField extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    final allowDownload = context.select((ShareableLinkState s) => s.formPayload['allowDownload'] ?? false);
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        CheckboxListTile(
          contentPadding: EdgeInsets.all(0),
          title: text('Allow download', style: Theme.of(context).textTheme.subtitle1),
          value: allowDownload,
          onChanged: (value) {
            context.read<ShareableLinkState>().setFormValue('allowDownload', value);
          },
        ),
