...
class EntryUsersList extends StatelessWidget {
  const EntryUsersList(this.fileEntry, {
    Key key,
  }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    final List<FileEntryUser> entryUsers = context.select((DriveState s) => s.entryCache.get(fileEntry.id).users);
    return Expanded(
      child: ListView(
        children: [
          ...entryUsers.map((u) => EntryUserListTile(fileEntry: fileEntry, fileEntryUser: u))
        ],
      ),
    );
  }
