      ...
      appBar: AppBar(
        title: Column(
          ...
          children: [
            ...
            SizedBox(height: 2),
            text('Share ${fileEntry.type == 'folder' ? 'Folder' : 'File'}', size: 14),
          ],
        ),
      ),
      body: Container(
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                text('Invite collaborators', style: Theme.of(context).textTheme.subtitle1),
                InitialPermissionPopupButton(),
              ],
            ),
            SizedBox(height: 5),
            EmailChipsInput(fileEntry, onChanged: (value) {
              context.read<ManageEntryUsersState>().toggleShareButtonState(value.isNotEmpty);
            }, key: emailChipInputKey),
            SizedBox(height: 30),
            text('Who has access', style: Theme.of(context).textTheme.subtitle1),
  ...
  Widget build(BuildContext context) {
    ...
    return PopupMenuButton<EntryPermissionName>(
      child: Padding(
        ...
        child: text(SharedEntryPermissions.displayNames[selectedPermission], color: primaryColor),
      ),
      onSelected: (name) {
        context.read<ManageEntryUsersState>().setPermissionsForNewUsers(SharedEntryPermissions.fromName(name));
      },
...
class ShareButton extends StatelessWidget {
  const ShareButton(this.fileEntry, {
    Key key,
  }) : super(key: key);
  ...
  @override
  Widget build(BuildContext context) {
    final loading = context.select((ManageEntryUsersState s) => s.loading != false);
    final enabled = context.select((ManageEntryUsersState s) => s.shareButtonEnabled != false);
    return Container(
      child: RaisedButton(
        child: text('Share'),
  ...
  _share(BuildContext context) async {
    ...
    if (chipInputState.chips.isEmpty) {
      return showSnackBar(trans('Enter at least one email address to share with.'), context);
    }

    chipInputState.addChip(chipInputState.controller.text);
    chipInputState.focusNode.unfocus();
    try {
      await state.addUsers([fileEntry.id], chipInputState.chips.map((c) => c.email).toList());
