...
class FilePreviewState with ChangeNotifier {
  ...
  FilePreviewState(
    ...
    downloadLocation = localStorage.temporary.scopedToSubDir(FilePreviewState.DOWNLOAD_PATH);
  }

  final OfflinedEntries offlinedEntries;
  final LocalStorage localStorage;
  final FileEntriesApi api;
  LocalStorageAdapter downloadLocation;
  FileEntry fileEntry;

  Future<File> getLocallyStoredFile(BuildContext context, {FileEntry entry, bool download = false}) async {
  ...
  _downloadFileLocally(BuildContext context, FileEntry entry) async {
    ...
    LoadingDialog.show(child: LoadingFileDialog(entry, downloadStreamCtrl.stream), dismissible: true).then((_) {
      dialogOpen = false;
  ...
  Future<File> _downloadFile(CancelToken cancelToken, FileEntry entry, ProgressCallback callback) async {
    final locallyStoredEntry = _getDownloadLocation(entry);
    final client = new Dio();
    await client.download(api.downloadUrl([entry]), locallyStoredEntry.path, cancelToken: cancelToken, onReceiveProgress: callback);
    return locallyStoredEntry;
  }

  File _getDownloadLocation(FileEntry entry) {
    return File('${downloadLocation.rootDir.path}/${basenameWithoutExtension(entry.name)}.${entry.extension}');
  }
}