...
typedef UploadManagerCallback<T> = void Function(UploadTaskStatus status, int progress, String taskId, FileEntry fileEntry, BackendError err);
...
backgroundHandler() {
  ...
  uploader.progress.listen((e) {
    ...
    try {
      send.send([e.taskId, e.status, e.progress, null]);
    } catch (_) {}
  });
  uploader.result.listen((e) {
    final SendPort send = IsolateNameServer.lookupPortByName('uploader_send_port');
    try {
      send.send([e.taskId, e.status, 100, e.response]);
    } catch(_) {}
  });
}
...
class UploadManager {
  ...
  Future<String> enqueue(FileUpload upload, {UploadManagerCallback callback}) async {
    final uploadId = await _uploader.enqueue(
  ...
  void _bindBackgroundIsolate() {
    bool isSuccess = IsolateNameServer.registerPortWithName(_port.sendPort, 'uploader_send_port');
    if (!isSuccess) {
    ...
    _port.listen((dynamic data) {
      ...
      if (callback != null) {
        ...
        final fileEntry = response["fileEntry"] == null ? null : FileEntry.fromJson(response["fileEntry"]);
        final err = status == UploadTaskStatus.failed ? BackendError(errResponse: response) : null;
