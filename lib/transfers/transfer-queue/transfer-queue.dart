...
class TransferQueue with ChangeNotifier {
  ...
  TransferQueue(this.notifications, this.downloader, this.uploader, this.localStorage) {
    _restoreQueue();
  ...
  Future<String> addDownload(FileEntry fileEntry, String destination, {FileTransferType type = FileTransferType.download}) async {
    ...
    if (taskId != null) {
      queue = {
        ...queue,
        fingerprint: DownloadTransfer(fileEntry, fingerprint, taskId, type: type),
      };
  ...
  updateProgress(String fingerprint, TransferProgress progress) {
    ...
    if (_lastProgressUpdate.progress != progress && transfer != null) {
      ...
      if (_lastProgressUpdate.time == null || progress.percentage == 100 || now.difference(_lastProgressUpdate.time).inMilliseconds > 350) {
  ...
  resumeTransfer(String fingerprint, {bool restart}) async {
    ...
    if (transfer != null) {
      ...
      if (restart) {
        newTaskId = transfer is UploadTransfer ? uploader.restart(transfer) : downloader.restart(transfer);
      } else {
        newTaskId = transfer is UploadTransfer ? uploader.resume(transfer) : downloader.resume(transfer);
      }
  ...
  _restoreQueue() async {
    if (await localStorage.permanent.exists(fileName)) {
      Map<String, dynamic> decoded = json.decode(await localStorage.permanent.get(fileName));
      decoded.forEach((fingerprint, decodedTransfer) {
        if (decodedTransfer['type'] == describeEnum(FileTransferType.upload)) {
          ...
          queue[fingerprint] = transfer;
          if (transfer.status != TransferQueueStatus.completed) {
            final callback = _createDownloadCallback(fingerprint, transfer.fileEntry);
  ...
  DownloadManagerCallback _createDownloadCallback(String fingerprint, FileEntry fileEntry) {
    return (DownloadTaskStatus status, int progress, _) {
      if (status == DownloadTaskStatus.complete) {
        ...
        errorTransfer(fingerprint, BackendError(message: 'File download failed'));
      }
    };
  }
  ...
  _updateNotification(FileTransferType type, {bool hideIfAllCompleted = false}) {
    if (Platform.isIOS) return;
    final notifId = notifIds[type];
    List<FileTransfer> queue = this.queue.values.where((e) => e.type == type).toList();
    List<FileTransfer> pending = queue.where((e) => e.status != TransferQueueStatus.completed).toList();
    String payload = json.encode({'notifId': NotificationType.transferProgress});
    ...
    // all paused
      ...
      notifications.notify(message, body: 'Paused', progress: 0, localId: notifId, payload: payload);
    }
    ...
    // some are still in progress
      ...
      final message = pending.length == 1
          ? '${action.capitalize()}ing "${pending.first.displayName}"'
          : '${action.capitalize()}ing ${pending.length} files';
      final body = filesize(pending.fold(0, (prev, curr) => prev + curr.progress.bytesLeft));
      final progress = pending.fold(0, (prev, curr) => (prev + curr.progress.percentage) / pending.length).floor();
      notifications.notify(message, body: body, progress: progress, localId: notifId, payload: payload);
    }
  }
}
